...
int main(int argc, char** argv) {
    ...
    instr_count -= 2; // remove EOF character count

    byte tape[TAPE_LEN] = {0};
    int tapeptr = 0; // memory pointer
    int instptr = 0; // instruction pointer
    int tapemax = 0; // maximum used memory pointer
    int match;       // store pointer to [ or ] for error messages

    while(1) {
        switch(instr[instptr]) {
            case '+':
                tape[tapeptr]++;
                instptr++;
                break;
            ...
            case '>':
                tapeptr++;
                if (tapeptr > tapemax) tapemax = tapeptr;
                if (tapeptr > TAPE_LEN) {
                    fprintf(stderr, "Tape overflow at instptr %d\n", instptr - 1);
                    return 1;
                }
                instptr++;
                break;
            ...
            case '<':
                tapeptr--;
                if (tapeptr < 0) {
                    fprintf(stderr, "Tape underflow at instptr %d\n", instptr + 1);
                    return 1;
                }
                instptr++;
                break;
            ...
            case '[':
                ...
                if (tape[tapeptr] == 0) {
                    while(instr[instptr] != ']') {
                        instptr++;
                        if (instptr > TAPE_LEN) {
                            fprintf(stderr, "No matching ] to [ at instptr %d\n", match);
                            return 1;
                        }
                    }
                }
            ...
            default:
                printf("Invalid bf character: %c\n", instr[instptr]);
                return 1;
        }
        if (instptr == instr_count) break;
    }

    printf("\n");
    for (int ii = 0; ii <= tapemax; ii++) {
        printf("%x ", tape[ii]);
    }
    printf("\n");
